...
main(void)
{
  ...
  if(fork() > 0)
    ...
    1019:	e8 e4 02 00 00       	call   1302 <sleep>
  exit();
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    ...
    1359:	0f b6 92 2d 17 00 00 	movzbl 0x172d(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    14ef:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
    14f1:	b8 26 17 00 00       	mov    $0x1726,%eax
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    15b1:	a1 a4 19 00 00       	mov    0x19a4,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    15f9:	a3 a4 19 00 00       	mov    %eax,0x19a4
}
    15fe:	5b                   	pop    %ebx
    15ff:	5e                   	pop    %esi
    1600:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    1622:	a3 a4 19 00 00       	mov    %eax,0x19a4
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    164c:	8b 1d a4 19 00 00    	mov    0x19a4,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    168b:	3b 15 a4 19 00 00    	cmp    0x19a4,%edx
...
morecore(uint nu)
{
  ...
  return freep;
    16c2:	8b 15 a4 19 00 00    	mov    0x19a4,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    16e4:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
    16e7:	89 15 a4 19 00 00    	mov    %edx,0x19a4
      return (void*)(p + 1);
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    16fe:	c7 05 a4 19 00 00 a8 	movl   $0x19a8,0x19a4
    1705:	19 00 00 
    base.s.size = 0;
    1708:	ba a8 19 00 00       	mov    $0x19a8,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    170d:	c7 05 a8 19 00 00 a8 	movl   $0x19a8,0x19a8
    1714:	19 00 00 
    base.s.size = 0;
    1717:	c7 05 ac 19 00 00 00 	movl   $0x0,0x19ac
    171e:	00 00 00 
