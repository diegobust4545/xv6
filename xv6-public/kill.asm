...
main(int argc, char **argv)
{
  ...
  if(argc < 2){
    printf(2, "usage: kill pid...\n");
    103f:	c7 44 24 04 56 17 00 	movl   $0x1756,0x4(%esp)
    1046:	00 
    1047:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
    104e:	e8 9d 03 00 00       	call   13f0 <printf>
    exit();
...
# Here
SYSCALL(getcount)
    130a:	b8 16 00 00 00       	mov    $0x16,%eax
    130f:	cd 40                	int    $0x40
    1311:	c3                   	ret    
...
# Here 2
SYSCALL(getpinfo)
    1312:	b8 17 00 00 00       	mov    $0x17,%eax
    1317:	cd 40                	int    $0x40
    1319:	c3                   	ret    
...
# Here 3
SYSCALL(uppriority)
    131a:	b8 18 00 00 00       	mov    $0x18,%eax
    131f:	cd 40                	int    $0x40
    1321:	c3                   	ret    

00001322 <dereference>:
# Here 4
SYSCALL(dereference)
    1322:	b8 19 00 00 00       	mov    $0x19,%eax
    1327:	cd 40                	int    $0x40
    1329:	c3                   	ret    
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    ...
    1389:	0f b6 92 71 17 00 00 	movzbl 0x1771(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
  write(fd, &c, 1);
    ...
    13ca:	e8 d3 fe ff ff       	call   12a2 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
    1436:	e8 67 fe ff ff       	call   12a2 <write>
...
putc(int fd, char c)
{
  write(fd, &c, 1);
    ...
    14a6:	e8 f7 fd ff ff       	call   12a2 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
    14c3:	e8 da fd ff ff       	call   12a2 <write>
  int c, i, state;
  uint *ap;
  ...
  for(i = 0; fmt[i]; i++){
    ...
    151f:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
    1521:	b8 6a 17 00 00       	mov    $0x176a,%eax
...
putc(int fd, char c)
{
  write(fd, &c, 1);
    ...
    1550:	e8 4d fd ff ff       	call   12a2 <write>
      } else if(c == 's'){
        s = (char*)*ap;
        ap++;
        if(s == 0)
...
putc(int fd, char c)
{
  write(fd, &c, 1);
    ...
    1580:	e8 1d fd ff ff       	call   12a2 <write>
...
putc(int fd, char c)
{
  write(fd, &c, 1);
    ...
    15d0:	e8 cd fc ff ff       	call   12a2 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    15e1:	a1 ec 19 00 00       	mov    0x19ec,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    1629:	a3 ec 19 00 00       	mov    %eax,0x19ec
}
    162e:	5b                   	pop    %ebx
    162f:	5e                   	pop    %esi
    1630:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    1652:	a3 ec 19 00 00       	mov    %eax,0x19ec
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    167c:	8b 1d ec 19 00 00    	mov    0x19ec,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    16bb:	3b 15 ec 19 00 00    	cmp    0x19ec,%edx
...
morecore(uint nu)
{
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    16da:	e8 4b fc ff ff       	call   132a <sbrk>
  if(p == (char*)-1)
  ...
  return freep;
    16f2:	8b 15 ec 19 00 00    	mov    0x19ec,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    1714:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
    1717:	89 15 ec 19 00 00    	mov    %edx,0x19ec
      return (void*)(p + 1);
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    172e:	c7 05 ec 19 00 00 f0 	movl   $0x19f0,0x19ec
    1735:	19 00 00 
    base.s.size = 0;
    1738:	ba f0 19 00 00       	mov    $0x19f0,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    173d:	c7 05 f0 19 00 00 f0 	movl   $0x19f0,0x19f0
    1744:	19 00 00 
    base.s.size = 0;
    1747:	c7 05 f4 19 00 00 00 	movl   $0x0,0x19f4
    174e:	00 00 00 
