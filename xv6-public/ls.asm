
_ls:     file format elf32-i386


Disassembly of section .text:

00001000 <main>:
  close(fd);
}

int
main(int argc, char *argv[])
{
    1000:	55                   	push   %ebp
    1001:	89 e5                	mov    %esp,%ebp
    1003:	57                   	push   %edi
    1004:	56                   	push   %esi
    1005:	53                   	push   %ebx
  int i;

  if(argc < 2){
    1006:	bb 01 00 00 00       	mov    $0x1,%ebx
  close(fd);
}

int
main(int argc, char *argv[])
{
    100b:	83 e4 f0             	and    $0xfffffff0,%esp
    100e:	83 ec 10             	sub    $0x10,%esp
    1011:	8b 75 08             	mov    0x8(%ebp),%esi
    1014:	8b 7d 0c             	mov    0xc(%ebp),%edi
  int i;

  if(argc < 2){
    1017:	83 fe 01             	cmp    $0x1,%esi
    101a:	7e 1b                	jle    1037 <main+0x37>
    101c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    ls(".");
    exit();
  }
  for(i=1; i<argc; i++)
    ls(argv[i]);
    1020:	8b 04 9f             	mov    (%edi,%ebx,4),%eax

  if(argc < 2){
    ls(".");
    exit();
  }
  for(i=1; i<argc; i++)
    1023:	83 c3 01             	add    $0x1,%ebx
    ls(argv[i]);
    1026:	89 04 24             	mov    %eax,(%esp)
    1029:	e8 c2 00 00 00       	call   10f0 <ls>

  if(argc < 2){
    ls(".");
    exit();
  }
  for(i=1; i<argc; i++)
    102e:	39 f3                	cmp    %esi,%ebx
    1030:	75 ee                	jne    1020 <main+0x20>
    ls(argv[i]);
  exit();
    1032:	e8 6b 05 00 00       	call   15a2 <exit>
main(int argc, char *argv[])
{
  int i;

  if(argc < 2){
    ls(".");
    1037:	c7 04 24 be 1a 00 00 	movl   $0x1abe,(%esp)
    103e:	e8 ad 00 00 00       	call   10f0 <ls>
    exit();
    1043:	e8 5a 05 00 00       	call   15a2 <exit>
    1048:	66 90                	xchg   %ax,%ax
    104a:	66 90                	xchg   %ax,%ax
    104c:	66 90                	xchg   %ax,%ax
    104e:	66 90                	xchg   %ax,%ax

00001050 <fmtname>:
#include "user.h"
#include "fs.h"

char*
fmtname(char *path)
{
    1050:	55                   	push   %ebp
    1051:	89 e5                	mov    %esp,%ebp
    1053:	56                   	push   %esi
    1054:	53                   	push   %ebx
    1055:	83 ec 10             	sub    $0x10,%esp
    1058:	8b 5d 08             	mov    0x8(%ebp),%ebx
  static char buf[DIRSIZ+1];
  char *p;

  // Find first character after last slash.
  for(p=path+strlen(path); p >= path && *p != '/'; p--)
    105b:	89 1c 24             	mov    %ebx,(%esp)
    105e:	e8 9d 03 00 00       	call   1400 <strlen>
    1063:	01 d8                	add    %ebx,%eax
    1065:	73 10                	jae    1077 <fmtname+0x27>
    1067:	eb 13                	jmp    107c <fmtname+0x2c>
    1069:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    1070:	83 e8 01             	sub    $0x1,%eax
    1073:	39 c3                	cmp    %eax,%ebx
    1075:	77 05                	ja     107c <fmtname+0x2c>
    1077:	80 38 2f             	cmpb   $0x2f,(%eax)
    107a:	75 f4                	jne    1070 <fmtname+0x20>
    ;
  p++;
    107c:	8d 58 01             	lea    0x1(%eax),%ebx

  // Return blank-padded name.
  if(strlen(p) >= DIRSIZ)
    107f:	89 1c 24             	mov    %ebx,(%esp)
    1082:	e8 79 03 00 00       	call   1400 <strlen>
    1087:	83 f8 0d             	cmp    $0xd,%eax
    108a:	77 53                	ja     10df <fmtname+0x8f>
    return p;
  memmove(buf, p, strlen(p));
    108c:	89 1c 24             	mov    %ebx,(%esp)
    108f:	e8 6c 03 00 00       	call   1400 <strlen>
    1094:	89 5c 24 04          	mov    %ebx,0x4(%esp)
    1098:	c7 04 24 a8 1d 00 00 	movl   $0x1da8,(%esp)
    109f:	89 44 24 08          	mov    %eax,0x8(%esp)
    10a3:	e8 c8 04 00 00       	call   1570 <memmove>
  memset(buf+strlen(p), ' ', DIRSIZ-strlen(p));
    10a8:	89 1c 24             	mov    %ebx,(%esp)
    10ab:	e8 50 03 00 00       	call   1400 <strlen>
    10b0:	89 1c 24             	mov    %ebx,(%esp)
  return buf;
    10b3:	bb a8 1d 00 00       	mov    $0x1da8,%ebx

  // Return blank-padded name.
  if(strlen(p) >= DIRSIZ)
    return p;
  memmove(buf, p, strlen(p));
  memset(buf+strlen(p), ' ', DIRSIZ-strlen(p));
    10b8:	89 c6                	mov    %eax,%esi
    10ba:	e8 41 03 00 00       	call   1400 <strlen>
    10bf:	ba 0e 00 00 00       	mov    $0xe,%edx
    10c4:	29 f2                	sub    %esi,%edx
    10c6:	89 54 24 08          	mov    %edx,0x8(%esp)
    10ca:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
    10d1:	00 
    10d2:	05 a8 1d 00 00       	add    $0x1da8,%eax
    10d7:	89 04 24             	mov    %eax,(%esp)
    10da:	e8 51 03 00 00       	call   1430 <memset>
  return buf;
}
    10df:	83 c4 10             	add    $0x10,%esp
    10e2:	89 d8                	mov    %ebx,%eax
    10e4:	5b                   	pop    %ebx
    10e5:	5e                   	pop    %esi
    10e6:	5d                   	pop    %ebp
    10e7:	c3                   	ret    
    10e8:	90                   	nop
    10e9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

000010f0 <ls>:

void
ls(char *path)
{
    10f0:	55                   	push   %ebp
    10f1:	89 e5                	mov    %esp,%ebp
    10f3:	57                   	push   %edi
    10f4:	56                   	push   %esi
    10f5:	53                   	push   %ebx
    10f6:	81 ec 6c 02 00 00    	sub    $0x26c,%esp
    10fc:	8b 7d 08             	mov    0x8(%ebp),%edi
  char buf[512], *p;
  int fd;
  struct dirent de;
  struct stat st;

  if((fd = open(path, 0)) < 0){
    10ff:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
    1106:	00 
    1107:	89 3c 24             	mov    %edi,(%esp)
    110a:	e8 d3 04 00 00       	call   15e2 <open>
    110f:	85 c0                	test   %eax,%eax
    1111:	89 c3                	mov    %eax,%ebx
    1113:	0f 88 c7 01 00 00    	js     12e0 <ls+0x1f0>
    printf(2, "ls: cannot open %s\n", path);
    return;
  }

  if(fstat(fd, &st) < 0){
    1119:	8d b5 d4 fd ff ff    	lea    -0x22c(%ebp),%esi
    111f:	89 74 24 04          	mov    %esi,0x4(%esp)
    1123:	89 04 24             	mov    %eax,(%esp)
    1126:	e8 cf 04 00 00       	call   15fa <fstat>
    112b:	85 c0                	test   %eax,%eax
    112d:	0f 88 f5 01 00 00    	js     1328 <ls+0x238>
    printf(2, "ls: cannot stat %s\n", path);
    close(fd);
    return;
  }

  switch(st.type){
    1133:	0f b7 85 d4 fd ff ff 	movzwl -0x22c(%ebp),%eax
    113a:	66 83 f8 01          	cmp    $0x1,%ax
    113e:	74 68                	je     11a8 <ls+0xb8>
    1140:	66 83 f8 02          	cmp    $0x2,%ax
    1144:	75 48                	jne    118e <ls+0x9e>
  case T_FILE:
    printf(1, "%s %d %d %d\n", fmtname(path), st.type, st.ino, st.size);
    1146:	8b 95 e4 fd ff ff    	mov    -0x21c(%ebp),%edx
    114c:	89 3c 24             	mov    %edi,(%esp)
    114f:	8b b5 dc fd ff ff    	mov    -0x224(%ebp),%esi
    1155:	89 95 b4 fd ff ff    	mov    %edx,-0x24c(%ebp)
    115b:	e8 f0 fe ff ff       	call   1050 <fmtname>
    1160:	8b 95 b4 fd ff ff    	mov    -0x24c(%ebp),%edx
    1166:	89 74 24 10          	mov    %esi,0x10(%esp)
    116a:	c7 44 24 0c 02 00 00 	movl   $0x2,0xc(%esp)
    1171:	00 
    1172:	c7 44 24 04 9e 1a 00 	movl   $0x1a9e,0x4(%esp)
    1179:	00 
    117a:	89 54 24 14          	mov    %edx,0x14(%esp)
    117e:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
    1185:	89 44 24 08          	mov    %eax,0x8(%esp)
    1189:	e8 82 05 00 00       	call   1710 <printf>
      }
      printf(1, "%s %d %d %d\n", fmtname(buf), st.type, st.ino, st.size);
    }
    break;
  }
  close(fd);
    118e:	89 1c 24             	mov    %ebx,(%esp)
    1191:	e8 34 04 00 00       	call   15ca <close>
}
    1196:	81 c4 6c 02 00 00    	add    $0x26c,%esp
    119c:	5b                   	pop    %ebx
    119d:	5e                   	pop    %esi
    119e:	5f                   	pop    %edi
    119f:	5d                   	pop    %ebp
    11a0:	c3                   	ret    
    11a1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  case T_FILE:
    printf(1, "%s %d %d %d\n", fmtname(path), st.type, st.ino, st.size);
    break;

  case T_DIR:
    if(strlen(path) + 1 + DIRSIZ + 1 > sizeof buf){
    11a8:	89 3c 24             	mov    %edi,(%esp)
    11ab:	e8 50 02 00 00       	call   1400 <strlen>
    11b0:	83 c0 10             	add    $0x10,%eax
    11b3:	3d 00 02 00 00       	cmp    $0x200,%eax
    11b8:	0f 87 4a 01 00 00    	ja     1308 <ls+0x218>
      printf(1, "ls: path too long\n");
      break;
    }
    strcpy(buf, path);
    11be:	8d 85 e8 fd ff ff    	lea    -0x218(%ebp),%eax
    11c4:	89 7c 24 04          	mov    %edi,0x4(%esp)
    11c8:	8d bd c4 fd ff ff    	lea    -0x23c(%ebp),%edi
    11ce:	89 04 24             	mov    %eax,(%esp)
    11d1:	e8 aa 01 00 00       	call   1380 <strcpy>
    p = buf+strlen(buf);
    11d6:	8d 85 e8 fd ff ff    	lea    -0x218(%ebp),%eax
    11dc:	89 04 24             	mov    %eax,(%esp)
    11df:	e8 1c 02 00 00       	call   1400 <strlen>
    11e4:	8d 8d e8 fd ff ff    	lea    -0x218(%ebp),%ecx
    11ea:	01 c8                	add    %ecx,%eax
    *p++ = '/';
    11ec:	8d 48 01             	lea    0x1(%eax),%ecx
    if(strlen(path) + 1 + DIRSIZ + 1 > sizeof buf){
      printf(1, "ls: path too long\n");
      break;
    }
    strcpy(buf, path);
    p = buf+strlen(buf);
    11ef:	89 85 a8 fd ff ff    	mov    %eax,-0x258(%ebp)
    *p++ = '/';
    11f5:	89 8d a4 fd ff ff    	mov    %ecx,-0x25c(%ebp)
    11fb:	c6 00 2f             	movb   $0x2f,(%eax)
    11fe:	66 90                	xchg   %ax,%ax
    while(read(fd, &de, sizeof(de)) == sizeof(de)){
    1200:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
    1207:	00 
    1208:	89 7c 24 04          	mov    %edi,0x4(%esp)
    120c:	89 1c 24             	mov    %ebx,(%esp)
    120f:	e8 a6 03 00 00       	call   15ba <read>
    1214:	83 f8 10             	cmp    $0x10,%eax
    1217:	0f 85 71 ff ff ff    	jne    118e <ls+0x9e>
      if(de.inum == 0)
    121d:	66 83 bd c4 fd ff ff 	cmpw   $0x0,-0x23c(%ebp)
    1224:	00 
    1225:	74 d9                	je     1200 <ls+0x110>
        continue;
      memmove(p, de.name, DIRSIZ);
    1227:	8d 85 c6 fd ff ff    	lea    -0x23a(%ebp),%eax
    122d:	89 44 24 04          	mov    %eax,0x4(%esp)
    1231:	8b 85 a4 fd ff ff    	mov    -0x25c(%ebp),%eax
    1237:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
    123e:	00 
    123f:	89 04 24             	mov    %eax,(%esp)
    1242:	e8 29 03 00 00       	call   1570 <memmove>
      p[DIRSIZ] = 0;
    1247:	8b 85 a8 fd ff ff    	mov    -0x258(%ebp),%eax
    124d:	c6 40 0f 00          	movb   $0x0,0xf(%eax)
      if(stat(buf, &st) < 0){
    1251:	8d 85 e8 fd ff ff    	lea    -0x218(%ebp),%eax
    1257:	89 74 24 04          	mov    %esi,0x4(%esp)
    125b:	89 04 24             	mov    %eax,(%esp)
    125e:	e8 8d 02 00 00       	call   14f0 <stat>
    1263:	85 c0                	test   %eax,%eax
    1265:	0f 88 e5 00 00 00    	js     1350 <ls+0x260>
        printf(1, "ls: cannot stat %s\n", buf);
        continue;
      }
      printf(1, "%s %d %d %d\n", fmtname(buf), st.type, st.ino, st.size);
    126b:	0f bf 95 d4 fd ff ff 	movswl -0x22c(%ebp),%edx
    1272:	8b 8d e4 fd ff ff    	mov    -0x21c(%ebp),%ecx
    1278:	8b 85 dc fd ff ff    	mov    -0x224(%ebp),%eax
    127e:	89 95 b0 fd ff ff    	mov    %edx,-0x250(%ebp)
    1284:	8d 95 e8 fd ff ff    	lea    -0x218(%ebp),%edx
    128a:	89 14 24             	mov    %edx,(%esp)
    128d:	89 8d ac fd ff ff    	mov    %ecx,-0x254(%ebp)
    1293:	89 85 b4 fd ff ff    	mov    %eax,-0x24c(%ebp)
    1299:	e8 b2 fd ff ff       	call   1050 <fmtname>
    129e:	8b 8d ac fd ff ff    	mov    -0x254(%ebp),%ecx
    12a4:	8b 95 b0 fd ff ff    	mov    -0x250(%ebp),%edx
    12aa:	c7 44 24 04 9e 1a 00 	movl   $0x1a9e,0x4(%esp)
    12b1:	00 
    12b2:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
    12b9:	89 4c 24 14          	mov    %ecx,0x14(%esp)
    12bd:	8b 8d b4 fd ff ff    	mov    -0x24c(%ebp),%ecx
    12c3:	89 54 24 0c          	mov    %edx,0xc(%esp)
    12c7:	89 44 24 08          	mov    %eax,0x8(%esp)
    12cb:	89 4c 24 10          	mov    %ecx,0x10(%esp)
    12cf:	e8 3c 04 00 00       	call   1710 <printf>
    12d4:	e9 27 ff ff ff       	jmp    1200 <ls+0x110>
    12d9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  int fd;
  struct dirent de;
  struct stat st;

  if((fd = open(path, 0)) < 0){
    printf(2, "ls: cannot open %s\n", path);
    12e0:	89 7c 24 08          	mov    %edi,0x8(%esp)
    12e4:	c7 44 24 04 76 1a 00 	movl   $0x1a76,0x4(%esp)
    12eb:	00 
    12ec:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
    12f3:	e8 18 04 00 00       	call   1710 <printf>
      printf(1, "%s %d %d %d\n", fmtname(buf), st.type, st.ino, st.size);
    }
    break;
  }
  close(fd);
}
    12f8:	81 c4 6c 02 00 00    	add    $0x26c,%esp
    12fe:	5b                   	pop    %ebx
    12ff:	5e                   	pop    %esi
    1300:	5f                   	pop    %edi
    1301:	5d                   	pop    %ebp
    1302:	c3                   	ret    
    1303:	90                   	nop
    1304:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    printf(1, "%s %d %d %d\n", fmtname(path), st.type, st.ino, st.size);
    break;

  case T_DIR:
    if(strlen(path) + 1 + DIRSIZ + 1 > sizeof buf){
      printf(1, "ls: path too long\n");
    1308:	c7 44 24 04 ab 1a 00 	movl   $0x1aab,0x4(%esp)
    130f:	00 
    1310:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
    1317:	e8 f4 03 00 00       	call   1710 <printf>
      break;
    131c:	e9 6d fe ff ff       	jmp    118e <ls+0x9e>
    1321:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    printf(2, "ls: cannot open %s\n", path);
    return;
  }

  if(fstat(fd, &st) < 0){
    printf(2, "ls: cannot stat %s\n", path);
    1328:	89 7c 24 08          	mov    %edi,0x8(%esp)
    132c:	c7 44 24 04 8a 1a 00 	movl   $0x1a8a,0x4(%esp)
    1333:	00 
    1334:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
    133b:	e8 d0 03 00 00       	call   1710 <printf>
    close(fd);
    1340:	89 1c 24             	mov    %ebx,(%esp)
    1343:	e8 82 02 00 00       	call   15ca <close>
    return;
    1348:	e9 49 fe ff ff       	jmp    1196 <ls+0xa6>
    134d:	8d 76 00             	lea    0x0(%esi),%esi
      if(de.inum == 0)
        continue;
      memmove(p, de.name, DIRSIZ);
      p[DIRSIZ] = 0;
      if(stat(buf, &st) < 0){
        printf(1, "ls: cannot stat %s\n", buf);
    1350:	8d 85 e8 fd ff ff    	lea    -0x218(%ebp),%eax
    1356:	89 44 24 08          	mov    %eax,0x8(%esp)
    135a:	c7 44 24 04 8a 1a 00 	movl   $0x1a8a,0x4(%esp)
    1361:	00 
    1362:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
    1369:	e8 a2 03 00 00       	call   1710 <printf>
        continue;
    136e:	e9 8d fe ff ff       	jmp    1200 <ls+0x110>
    1373:	66 90                	xchg   %ax,%ax
    1375:	66 90                	xchg   %ax,%ax
    1377:	66 90                	xchg   %ax,%ax
    1379:	66 90                	xchg   %ax,%ax
    137b:	66 90                	xchg   %ax,%ax
    137d:	66 90                	xchg   %ax,%ax
    137f:	90                   	nop

00001380 <strcpy>:
#include "user.h"
#include "x86.h"

char*
strcpy(char *s, char *t)
{
    1380:	55                   	push   %ebp
    1381:	89 e5                	mov    %esp,%ebp
    1383:	8b 45 08             	mov    0x8(%ebp),%eax
    1386:	8b 4d 0c             	mov    0xc(%ebp),%ecx
    1389:	53                   	push   %ebx
  char *os;

  os = s;
  while((*s++ = *t++) != 0)
    138a:	89 c2                	mov    %eax,%edx
    138c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    1390:	83 c1 01             	add    $0x1,%ecx
    1393:	0f b6 59 ff          	movzbl -0x1(%ecx),%ebx
    1397:	83 c2 01             	add    $0x1,%edx
    139a:	84 db                	test   %bl,%bl
    139c:	88 5a ff             	mov    %bl,-0x1(%edx)
    139f:	75 ef                	jne    1390 <strcpy+0x10>
    ;
  return os;
}
    13a1:	5b                   	pop    %ebx
    13a2:	5d                   	pop    %ebp
    13a3:	c3                   	ret    
    13a4:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    13aa:	8d bf 00 00 00 00    	lea    0x0(%edi),%edi

000013b0 <strcmp>:

int
strcmp(const char *p, const char *q)
{
    13b0:	55                   	push   %ebp
    13b1:	89 e5                	mov    %esp,%ebp
    13b3:	8b 55 08             	mov    0x8(%ebp),%edx
    13b6:	53                   	push   %ebx
    13b7:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  while(*p && *p == *q)
    13ba:	0f b6 02             	movzbl (%edx),%eax
    13bd:	84 c0                	test   %al,%al
    13bf:	74 2d                	je     13ee <strcmp+0x3e>
    13c1:	0f b6 19             	movzbl (%ecx),%ebx
    13c4:	38 d8                	cmp    %bl,%al
    13c6:	74 0e                	je     13d6 <strcmp+0x26>
    13c8:	eb 2b                	jmp    13f5 <strcmp+0x45>
    13ca:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    13d0:	38 c8                	cmp    %cl,%al
    13d2:	75 15                	jne    13e9 <strcmp+0x39>
    p++, q++;
    13d4:	89 d9                	mov    %ebx,%ecx
    13d6:	83 c2 01             	add    $0x1,%edx
}

int
strcmp(const char *p, const char *q)
{
  while(*p && *p == *q)
    13d9:	0f b6 02             	movzbl (%edx),%eax
    p++, q++;
    13dc:	8d 59 01             	lea    0x1(%ecx),%ebx
}

int
strcmp(const char *p, const char *q)
{
  while(*p && *p == *q)
    13df:	0f b6 49 01          	movzbl 0x1(%ecx),%ecx
    13e3:	84 c0                	test   %al,%al
    13e5:	75 e9                	jne    13d0 <strcmp+0x20>
    13e7:	31 c0                	xor    %eax,%eax
    p++, q++;
  return (uchar)*p - (uchar)*q;
    13e9:	29 c8                	sub    %ecx,%eax
}
    13eb:	5b                   	pop    %ebx
    13ec:	5d                   	pop    %ebp
    13ed:	c3                   	ret    
    13ee:	0f b6 09             	movzbl (%ecx),%ecx
}

int
strcmp(const char *p, const char *q)
{
  while(*p && *p == *q)
    13f1:	31 c0                	xor    %eax,%eax
    13f3:	eb f4                	jmp    13e9 <strcmp+0x39>
    13f5:	0f b6 cb             	movzbl %bl,%ecx
    13f8:	eb ef                	jmp    13e9 <strcmp+0x39>
    13fa:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

00001400 <strlen>:
  return (uchar)*p - (uchar)*q;
}

uint
strlen(char *s)
{
    1400:	55                   	push   %ebp
    1401:	89 e5                	mov    %esp,%ebp
    1403:	8b 4d 08             	mov    0x8(%ebp),%ecx
  int n;

  for(n = 0; s[n]; n++)
    1406:	80 39 00             	cmpb   $0x0,(%ecx)
    1409:	74 12                	je     141d <strlen+0x1d>
    140b:	31 d2                	xor    %edx,%edx
    140d:	8d 76 00             	lea    0x0(%esi),%esi
    1410:	83 c2 01             	add    $0x1,%edx
    1413:	80 3c 11 00          	cmpb   $0x0,(%ecx,%edx,1)
    1417:	89 d0                	mov    %edx,%eax
    1419:	75 f5                	jne    1410 <strlen+0x10>
    ;
  return n;
}
    141b:	5d                   	pop    %ebp
    141c:	c3                   	ret    
uint
strlen(char *s)
{
  int n;

  for(n = 0; s[n]; n++)
    141d:	31 c0                	xor    %eax,%eax
    ;
  return n;
}
    141f:	5d                   	pop    %ebp
    1420:	c3                   	ret    
    1421:	eb 0d                	jmp    1430 <memset>
    1423:	90                   	nop
    1424:	90                   	nop
    1425:	90                   	nop
    1426:	90                   	nop
    1427:	90                   	nop
    1428:	90                   	nop
    1429:	90                   	nop
    142a:	90                   	nop
    142b:	90                   	nop
    142c:	90                   	nop
    142d:	90                   	nop
    142e:	90                   	nop
    142f:	90                   	nop

00001430 <memset>:

void*
memset(void *dst, int c, uint n)
{
    1430:	55                   	push   %ebp
    1431:	89 e5                	mov    %esp,%ebp
    1433:	8b 55 08             	mov    0x8(%ebp),%edx
    1436:	57                   	push   %edi
}

static inline void
stosb(void *addr, int data, int cnt)
{
  asm volatile("cld; rep stosb" :
    1437:	8b 4d 10             	mov    0x10(%ebp),%ecx
    143a:	8b 45 0c             	mov    0xc(%ebp),%eax
    143d:	89 d7                	mov    %edx,%edi
    143f:	fc                   	cld    
    1440:	f3 aa                	rep stos %al,%es:(%edi)
  stosb(dst, c, n);
  return dst;
}
    1442:	89 d0                	mov    %edx,%eax
    1444:	5f                   	pop    %edi
    1445:	5d                   	pop    %ebp
    1446:	c3                   	ret    
    1447:	89 f6                	mov    %esi,%esi
    1449:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

00001450 <strchr>:

char*
strchr(const char *s, char c)
{
    1450:	55                   	push   %ebp
    1451:	89 e5                	mov    %esp,%ebp
    1453:	8b 45 08             	mov    0x8(%ebp),%eax
    1456:	53                   	push   %ebx
    1457:	8b 55 0c             	mov    0xc(%ebp),%edx
  for(; *s; s++)
    145a:	0f b6 18             	movzbl (%eax),%ebx
    145d:	84 db                	test   %bl,%bl
    145f:	74 1d                	je     147e <strchr+0x2e>
    if(*s == c)
    1461:	38 d3                	cmp    %dl,%bl
    1463:	89 d1                	mov    %edx,%ecx
    1465:	75 0d                	jne    1474 <strchr+0x24>
    1467:	eb 17                	jmp    1480 <strchr+0x30>
    1469:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    1470:	38 ca                	cmp    %cl,%dl
    1472:	74 0c                	je     1480 <strchr+0x30>
}

char*
strchr(const char *s, char c)
{
  for(; *s; s++)
    1474:	83 c0 01             	add    $0x1,%eax
    1477:	0f b6 10             	movzbl (%eax),%edx
    147a:	84 d2                	test   %dl,%dl
    147c:	75 f2                	jne    1470 <strchr+0x20>
    if(*s == c)
      return (char*)s;
  return 0;
    147e:	31 c0                	xor    %eax,%eax
}
    1480:	5b                   	pop    %ebx
    1481:	5d                   	pop    %ebp
    1482:	c3                   	ret    
    1483:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    1489:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

00001490 <gets>:

char*
gets(char *buf, int max)
{
    1490:	55                   	push   %ebp
    1491:	89 e5                	mov    %esp,%ebp
    1493:	57                   	push   %edi
    1494:	56                   	push   %esi
  int i, cc;
  char c;

  for(i=0; i+1 < max; ){
    1495:	31 f6                	xor    %esi,%esi
  return 0;
}

char*
gets(char *buf, int max)
{
    1497:	53                   	push   %ebx
    1498:	83 ec 2c             	sub    $0x2c,%esp
  int i, cc;
  char c;

  for(i=0; i+1 < max; ){
    cc = read(0, &c, 1);
    149b:	8d 7d e7             	lea    -0x19(%ebp),%edi
gets(char *buf, int max)
{
  int i, cc;
  char c;

  for(i=0; i+1 < max; ){
    149e:	eb 31                	jmp    14d1 <gets+0x41>
    cc = read(0, &c, 1);
    14a0:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
    14a7:	00 
    14a8:	89 7c 24 04          	mov    %edi,0x4(%esp)
    14ac:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
    14b3:	e8 02 01 00 00       	call   15ba <read>
    if(cc < 1)
    14b8:	85 c0                	test   %eax,%eax
    14ba:	7e 1d                	jle    14d9 <gets+0x49>
      break;
    buf[i++] = c;
    14bc:	0f b6 45 e7          	movzbl -0x19(%ebp),%eax
gets(char *buf, int max)
{
  int i, cc;
  char c;

  for(i=0; i+1 < max; ){
    14c0:	89 de                	mov    %ebx,%esi
    cc = read(0, &c, 1);
    if(cc < 1)
      break;
    buf[i++] = c;
    14c2:	8b 55 08             	mov    0x8(%ebp),%edx
    if(c == '\n' || c == '\r')
    14c5:	3c 0d                	cmp    $0xd,%al

  for(i=0; i+1 < max; ){
    cc = read(0, &c, 1);
    if(cc < 1)
      break;
    buf[i++] = c;
    14c7:	88 44 1a ff          	mov    %al,-0x1(%edx,%ebx,1)
    if(c == '\n' || c == '\r')
    14cb:	74 0c                	je     14d9 <gets+0x49>
    14cd:	3c 0a                	cmp    $0xa,%al
    14cf:	74 08                	je     14d9 <gets+0x49>
gets(char *buf, int max)
{
  int i, cc;
  char c;

  for(i=0; i+1 < max; ){
    14d1:	8d 5e 01             	lea    0x1(%esi),%ebx
    14d4:	3b 5d 0c             	cmp    0xc(%ebp),%ebx
    14d7:	7c c7                	jl     14a0 <gets+0x10>
      break;
    buf[i++] = c;
    if(c == '\n' || c == '\r')
      break;
  }
  buf[i] = '\0';
    14d9:	8b 45 08             	mov    0x8(%ebp),%eax
    14dc:	c6 04 30 00          	movb   $0x0,(%eax,%esi,1)
  return buf;
}
    14e0:	83 c4 2c             	add    $0x2c,%esp
    14e3:	5b                   	pop    %ebx
    14e4:	5e                   	pop    %esi
    14e5:	5f                   	pop    %edi
    14e6:	5d                   	pop    %ebp
    14e7:	c3                   	ret    
    14e8:	90                   	nop
    14e9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

000014f0 <stat>:

int
stat(char *n, struct stat *st)
{
    14f0:	55                   	push   %ebp
    14f1:	89 e5                	mov    %esp,%ebp
    14f3:	56                   	push   %esi
    14f4:	53                   	push   %ebx
    14f5:	83 ec 10             	sub    $0x10,%esp
  int fd;
  int r;

  fd = open(n, O_RDONLY);
    14f8:	8b 45 08             	mov    0x8(%ebp),%eax
    14fb:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
    1502:	00 
    1503:	89 04 24             	mov    %eax,(%esp)
    1506:	e8 d7 00 00 00       	call   15e2 <open>
  if(fd < 0)
    150b:	85 c0                	test   %eax,%eax
stat(char *n, struct stat *st)
{
  int fd;
  int r;

  fd = open(n, O_RDONLY);
    150d:	89 c3                	mov    %eax,%ebx
  if(fd < 0)
    150f:	78 27                	js     1538 <stat+0x48>
    return -1;
  r = fstat(fd, st);
    1511:	8b 45 0c             	mov    0xc(%ebp),%eax
    1514:	89 1c 24             	mov    %ebx,(%esp)
    1517:	89 44 24 04          	mov    %eax,0x4(%esp)
    151b:	e8 da 00 00 00       	call   15fa <fstat>
  close(fd);
    1520:	89 1c 24             	mov    %ebx,(%esp)
  int r;

  fd = open(n, O_RDONLY);
  if(fd < 0)
    return -1;
  r = fstat(fd, st);
    1523:	89 c6                	mov    %eax,%esi
  close(fd);
    1525:	e8 a0 00 00 00       	call   15ca <close>
  return r;
    152a:	89 f0                	mov    %esi,%eax
}
    152c:	83 c4 10             	add    $0x10,%esp
    152f:	5b                   	pop    %ebx
    1530:	5e                   	pop    %esi
    1531:	5d                   	pop    %ebp
    1532:	c3                   	ret    
    1533:	90                   	nop
    1534:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  int fd;
  int r;

  fd = open(n, O_RDONLY);
  if(fd < 0)
    return -1;
    1538:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
    153d:	eb ed                	jmp    152c <stat+0x3c>
    153f:	90                   	nop

00001540 <atoi>:
  return r;
}

int
atoi(const char *s)
{
    1540:	55                   	push   %ebp
    1541:	89 e5                	mov    %esp,%ebp
    1543:	8b 4d 08             	mov    0x8(%ebp),%ecx
    1546:	53                   	push   %ebx
  int n;

  n = 0;
  while('0' <= *s && *s <= '9')
    1547:	0f be 11             	movsbl (%ecx),%edx
    154a:	8d 42 d0             	lea    -0x30(%edx),%eax
    154d:	3c 09                	cmp    $0x9,%al
int
atoi(const char *s)
{
  int n;

  n = 0;
    154f:	b8 00 00 00 00       	mov    $0x0,%eax
  while('0' <= *s && *s <= '9')
    1554:	77 17                	ja     156d <atoi+0x2d>
    1556:	66 90                	xchg   %ax,%ax
    n = n*10 + *s++ - '0';
    1558:	83 c1 01             	add    $0x1,%ecx
    155b:	8d 04 80             	lea    (%eax,%eax,4),%eax
    155e:	8d 44 42 d0          	lea    -0x30(%edx,%eax,2),%eax
atoi(const char *s)
{
  int n;

  n = 0;
  while('0' <= *s && *s <= '9')
    1562:	0f be 11             	movsbl (%ecx),%edx
    1565:	8d 5a d0             	lea    -0x30(%edx),%ebx
    1568:	80 fb 09             	cmp    $0x9,%bl
    156b:	76 eb                	jbe    1558 <atoi+0x18>
    n = n*10 + *s++ - '0';
  return n;
}
    156d:	5b                   	pop    %ebx
    156e:	5d                   	pop    %ebp
    156f:	c3                   	ret    

00001570 <memmove>:

void*
memmove(void *vdst, void *vsrc, int n)
{
    1570:	55                   	push   %ebp
  char *dst, *src;

  dst = vdst;
  src = vsrc;
  while(n-- > 0)
    1571:	31 d2                	xor    %edx,%edx
  return n;
}

void*
memmove(void *vdst, void *vsrc, int n)
{
    1573:	89 e5                	mov    %esp,%ebp
    1575:	56                   	push   %esi
    1576:	8b 45 08             	mov    0x8(%ebp),%eax
    1579:	53                   	push   %ebx
    157a:	8b 5d 10             	mov    0x10(%ebp),%ebx
    157d:	8b 75 0c             	mov    0xc(%ebp),%esi
  char *dst, *src;

  dst = vdst;
  src = vsrc;
  while(n-- > 0)
    1580:	85 db                	test   %ebx,%ebx
    1582:	7e 12                	jle    1596 <memmove+0x26>
    1584:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    *dst++ = *src++;
    1588:	0f b6 0c 16          	movzbl (%esi,%edx,1),%ecx
    158c:	88 0c 10             	mov    %cl,(%eax,%edx,1)
    158f:	83 c2 01             	add    $0x1,%edx
{
  char *dst, *src;

  dst = vdst;
  src = vsrc;
  while(n-- > 0)
    1592:	39 da                	cmp    %ebx,%edx
    1594:	75 f2                	jne    1588 <memmove+0x18>
    *dst++ = *src++;
  return vdst;
}
    1596:	5b                   	pop    %ebx
    1597:	5e                   	pop    %esi
    1598:	5d                   	pop    %ebp
    1599:	c3                   	ret    

0000159a <fork>:
  name: \
    movl $SYS_ ## name, %eax; \
    int $T_SYSCALL; \
    ret

SYSCALL(fork)
    159a:	b8 01 00 00 00       	mov    $0x1,%eax
    159f:	cd 40                	int    $0x40
    15a1:	c3                   	ret    

000015a2 <exit>:
SYSCALL(exit)
    15a2:	b8 02 00 00 00       	mov    $0x2,%eax
    15a7:	cd 40                	int    $0x40
    15a9:	c3                   	ret    

000015aa <wait>:
SYSCALL(wait)
    15aa:	b8 03 00 00 00       	mov    $0x3,%eax
    15af:	cd 40                	int    $0x40
    15b1:	c3                   	ret    

000015b2 <pipe>:
SYSCALL(pipe)
    15b2:	b8 04 00 00 00       	mov    $0x4,%eax
    15b7:	cd 40                	int    $0x40
    15b9:	c3                   	ret    

000015ba <read>:
SYSCALL(read)
    15ba:	b8 05 00 00 00       	mov    $0x5,%eax
    15bf:	cd 40                	int    $0x40
    15c1:	c3                   	ret    

000015c2 <write>:
SYSCALL(write)
    15c2:	b8 10 00 00 00       	mov    $0x10,%eax
    15c7:	cd 40                	int    $0x40
    15c9:	c3                   	ret    

000015ca <close>:
SYSCALL(close)
    15ca:	b8 15 00 00 00       	mov    $0x15,%eax
    15cf:	cd 40                	int    $0x40
    15d1:	c3                   	ret    

000015d2 <kill>:
SYSCALL(kill)
    15d2:	b8 06 00 00 00       	mov    $0x6,%eax
    15d7:	cd 40                	int    $0x40
    15d9:	c3                   	ret    

000015da <exec>:
SYSCALL(exec)
    15da:	b8 07 00 00 00       	mov    $0x7,%eax
    15df:	cd 40                	int    $0x40
    15e1:	c3                   	ret    

000015e2 <open>:
SYSCALL(open)
    15e2:	b8 0f 00 00 00       	mov    $0xf,%eax
    15e7:	cd 40                	int    $0x40
    15e9:	c3                   	ret    

000015ea <mknod>:
SYSCALL(mknod)
    15ea:	b8 11 00 00 00       	mov    $0x11,%eax
    15ef:	cd 40                	int    $0x40
    15f1:	c3                   	ret    

000015f2 <unlink>:
SYSCALL(unlink)
    15f2:	b8 12 00 00 00       	mov    $0x12,%eax
    15f7:	cd 40                	int    $0x40
    15f9:	c3                   	ret    

000015fa <fstat>:
SYSCALL(fstat)
    15fa:	b8 08 00 00 00       	mov    $0x8,%eax
    15ff:	cd 40                	int    $0x40
    1601:	c3                   	ret    

00001602 <link>:
SYSCALL(link)
    1602:	b8 13 00 00 00       	mov    $0x13,%eax
    1607:	cd 40                	int    $0x40
    1609:	c3                   	ret    

0000160a <mkdir>:
SYSCALL(mkdir)
    160a:	b8 14 00 00 00       	mov    $0x14,%eax
    160f:	cd 40                	int    $0x40
    1611:	c3                   	ret    

00001612 <chdir>:
SYSCALL(chdir)
    1612:	b8 09 00 00 00       	mov    $0x9,%eax
    1617:	cd 40                	int    $0x40
    1619:	c3                   	ret    

0000161a <dup>:
SYSCALL(dup)
    161a:	b8 0a 00 00 00       	mov    $0xa,%eax
    161f:	cd 40                	int    $0x40
    1621:	c3                   	ret    

00001622 <getpid>:
SYSCALL(getpid)
    1622:	b8 0b 00 00 00       	mov    $0xb,%eax
    1627:	cd 40                	int    $0x40
    1629:	c3                   	ret    

0000162a <getcount>:

# Here
SYSCALL(getcount)
    162a:	b8 16 00 00 00       	mov    $0x16,%eax
    162f:	cd 40                	int    $0x40
    1631:	c3                   	ret    

00001632 <getpinfo>:
# Here 2
SYSCALL(getpinfo)
    1632:	b8 17 00 00 00       	mov    $0x17,%eax
    1637:	cd 40                	int    $0x40
    1639:	c3                   	ret    

0000163a <uppriority>:
# Here 3
SYSCALL(uppriority)
    163a:	b8 18 00 00 00       	mov    $0x18,%eax
    163f:	cd 40                	int    $0x40
    1641:	c3                   	ret    

00001642 <dereference>:
# Here 4
SYSCALL(dereference)
    1642:	b8 19 00 00 00       	mov    $0x19,%eax
    1647:	cd 40                	int    $0x40
    1649:	c3                   	ret    

0000164a <sbrk>:

SYSCALL(sbrk)
    164a:	b8 0c 00 00 00       	mov    $0xc,%eax
    164f:	cd 40                	int    $0x40
    1651:	c3                   	ret    

00001652 <sleep>:
SYSCALL(sleep)
    1652:	b8 0d 00 00 00       	mov    $0xd,%eax
    1657:	cd 40                	int    $0x40
    1659:	c3                   	ret    

0000165a <uptime>:
SYSCALL(uptime)
    165a:	b8 0e 00 00 00       	mov    $0xe,%eax
    165f:	cd 40                	int    $0x40
    1661:	c3                   	ret    
    1662:	66 90                	xchg   %ax,%ax
    1664:	66 90                	xchg   %ax,%ax
    1666:	66 90                	xchg   %ax,%ax
    1668:	66 90                	xchg   %ax,%ax
    166a:	66 90                	xchg   %ax,%ax
    166c:	66 90                	xchg   %ax,%ax
    166e:	66 90                	xchg   %ax,%ax

00001670 <printint>:
  write(fd, &c, 1);
}

static void
printint(int fd, int xx, int base, int sgn)
{
    1670:	55                   	push   %ebp
    1671:	89 e5                	mov    %esp,%ebp
    1673:	57                   	push   %edi
    1674:	56                   	push   %esi
    1675:	89 c6                	mov    %eax,%esi
    1677:	53                   	push   %ebx
    1678:	83 ec 4c             	sub    $0x4c,%esp
  char buf[16];
  int i, neg;
  uint x;

  neg = 0;
  if(sgn && xx < 0){
    167b:	8b 5d 08             	mov    0x8(%ebp),%ebx
    167e:	85 db                	test   %ebx,%ebx
    1680:	74 09                	je     168b <printint+0x1b>
    1682:	89 d0                	mov    %edx,%eax
    1684:	c1 e8 1f             	shr    $0x1f,%eax
    1687:	84 c0                	test   %al,%al
    1689:	75 75                	jne    1700 <printint+0x90>
    neg = 1;
    x = -xx;
  } else {
    x = xx;
    168b:	89 d0                	mov    %edx,%eax
  static char digits[] = "0123456789ABCDEF";
  char buf[16];
  int i, neg;
  uint x;

  neg = 0;
    168d:	c7 45 c4 00 00 00 00 	movl   $0x0,-0x3c(%ebp)
    1694:	89 75 c0             	mov    %esi,-0x40(%ebp)
    x = -xx;
  } else {
    x = xx;
  }

  i = 0;
    1697:	31 ff                	xor    %edi,%edi
    1699:	89 ce                	mov    %ecx,%esi
    169b:	8d 5d d7             	lea    -0x29(%ebp),%ebx
    169e:	eb 02                	jmp    16a2 <printint+0x32>
  do{
    buf[i++] = digits[x % base];
    16a0:	89 cf                	mov    %ecx,%edi
    16a2:	31 d2                	xor    %edx,%edx
    16a4:	f7 f6                	div    %esi
    16a6:	8d 4f 01             	lea    0x1(%edi),%ecx
    16a9:	0f b6 92 c7 1a 00 00 	movzbl 0x1ac7(%edx),%edx
  }while((x /= base) != 0);
    16b0:	85 c0                	test   %eax,%eax
    x = xx;
  }

  i = 0;
  do{
    buf[i++] = digits[x % base];
    16b2:	88 14 0b             	mov    %dl,(%ebx,%ecx,1)
  }while((x /= base) != 0);
    16b5:	75 e9                	jne    16a0 <printint+0x30>
  if(neg)
    16b7:	8b 55 c4             	mov    -0x3c(%ebp),%edx
    x = xx;
  }

  i = 0;
  do{
    buf[i++] = digits[x % base];
    16ba:	89 c8                	mov    %ecx,%eax
    16bc:	8b 75 c0             	mov    -0x40(%ebp),%esi
  }while((x /= base) != 0);
  if(neg)
    16bf:	85 d2                	test   %edx,%edx
    16c1:	74 08                	je     16cb <printint+0x5b>
    buf[i++] = '-';
    16c3:	8d 4f 02             	lea    0x2(%edi),%ecx
    16c6:	c6 44 05 d8 2d       	movb   $0x2d,-0x28(%ebp,%eax,1)

  while(--i >= 0)
    16cb:	8d 79 ff             	lea    -0x1(%ecx),%edi
    16ce:	66 90                	xchg   %ax,%ax
    16d0:	0f b6 44 3d d8       	movzbl -0x28(%ebp,%edi,1),%eax
    16d5:	83 ef 01             	sub    $0x1,%edi
#include "user.h"

static void
putc(int fd, char c)
{
  write(fd, &c, 1);
    16d8:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
    16df:	00 
    16e0:	89 5c 24 04          	mov    %ebx,0x4(%esp)
    16e4:	89 34 24             	mov    %esi,(%esp)
    16e7:	88 45 d7             	mov    %al,-0x29(%ebp)
    16ea:	e8 d3 fe ff ff       	call   15c2 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';

  while(--i >= 0)
    16ef:	83 ff ff             	cmp    $0xffffffff,%edi
    16f2:	75 dc                	jne    16d0 <printint+0x60>
    putc(fd, buf[i]);
}
    16f4:	83 c4 4c             	add    $0x4c,%esp
    16f7:	5b                   	pop    %ebx
    16f8:	5e                   	pop    %esi
    16f9:	5f                   	pop    %edi
    16fa:	5d                   	pop    %ebp
    16fb:	c3                   	ret    
    16fc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  uint x;

  neg = 0;
  if(sgn && xx < 0){
    neg = 1;
    x = -xx;
    1700:	89 d0                	mov    %edx,%eax
    1702:	f7 d8                	neg    %eax
  int i, neg;
  uint x;

  neg = 0;
  if(sgn && xx < 0){
    neg = 1;
    1704:	c7 45 c4 01 00 00 00 	movl   $0x1,-0x3c(%ebp)
    170b:	eb 87                	jmp    1694 <printint+0x24>
    170d:	8d 76 00             	lea    0x0(%esi),%esi

00001710 <printf>:
}

// Print to the given fd. Only understands %d, %x, %p, %s.
void
printf(int fd, char *fmt, ...)
{
    1710:	55                   	push   %ebp
    1711:	89 e5                	mov    %esp,%ebp
    1713:	57                   	push   %edi
  char *s;
  int c, i, state;
  uint *ap;

  state = 0;
    1714:	31 ff                	xor    %edi,%edi
}

// Print to the given fd. Only understands %d, %x, %p, %s.
void
printf(int fd, char *fmt, ...)
{
    1716:	56                   	push   %esi
    1717:	53                   	push   %ebx
    1718:	83 ec 3c             	sub    $0x3c,%esp
  int c, i, state;
  uint *ap;

  state = 0;
  ap = (uint*)(void*)&fmt + 1;
  for(i = 0; fmt[i]; i++){
    171b:	8b 5d 0c             	mov    0xc(%ebp),%ebx
  char *s;
  int c, i, state;
  uint *ap;

  state = 0;
  ap = (uint*)(void*)&fmt + 1;
    171e:	8d 45 10             	lea    0x10(%ebp),%eax
}

// Print to the given fd. Only understands %d, %x, %p, %s.
void
printf(int fd, char *fmt, ...)
{
    1721:	8b 75 08             	mov    0x8(%ebp),%esi
  char *s;
  int c, i, state;
  uint *ap;

  state = 0;
  ap = (uint*)(void*)&fmt + 1;
    1724:	89 45 d4             	mov    %eax,-0x2c(%ebp)
  for(i = 0; fmt[i]; i++){
    1727:	0f b6 13             	movzbl (%ebx),%edx
    172a:	83 c3 01             	add    $0x1,%ebx
    172d:	84 d2                	test   %dl,%dl
    172f:	75 39                	jne    176a <printf+0x5a>
    1731:	e9 c2 00 00 00       	jmp    17f8 <printf+0xe8>
    1736:	66 90                	xchg   %ax,%ax
    c = fmt[i] & 0xff;
    if(state == 0){
      if(c == '%'){
    1738:	83 fa 25             	cmp    $0x25,%edx
    173b:	0f 84 bf 00 00 00    	je     1800 <printf+0xf0>
#include "user.h"

static void
putc(int fd, char c)
{
  write(fd, &c, 1);
    1741:	8d 45 e2             	lea    -0x1e(%ebp),%eax
    1744:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
    174b:	00 
    174c:	89 44 24 04          	mov    %eax,0x4(%esp)
    1750:	89 34 24             	mov    %esi,(%esp)
    c = fmt[i] & 0xff;
    if(state == 0){
      if(c == '%'){
        state = '%';
      } else {
        putc(fd, c);
    1753:	88 55 e2             	mov    %dl,-0x1e(%ebp)
#include "user.h"

static void
putc(int fd, char c)
{
  write(fd, &c, 1);
    1756:	e8 67 fe ff ff       	call   15c2 <write>
    175b:	83 c3 01             	add    $0x1,%ebx
  int c, i, state;
  uint *ap;

  state = 0;
  ap = (uint*)(void*)&fmt + 1;
  for(i = 0; fmt[i]; i++){
    175e:	0f b6 53 ff          	movzbl -0x1(%ebx),%edx
    1762:	84 d2                	test   %dl,%dl
    1764:	0f 84 8e 00 00 00    	je     17f8 <printf+0xe8>
    c = fmt[i] & 0xff;
    if(state == 0){
    176a:	85 ff                	test   %edi,%edi
  uint *ap;

  state = 0;
  ap = (uint*)(void*)&fmt + 1;
  for(i = 0; fmt[i]; i++){
    c = fmt[i] & 0xff;
    176c:	0f be c2             	movsbl %dl,%eax
    if(state == 0){
    176f:	74 c7                	je     1738 <printf+0x28>
      if(c == '%'){
        state = '%';
      } else {
        putc(fd, c);
      }
    } else if(state == '%'){
    1771:	83 ff 25             	cmp    $0x25,%edi
    1774:	75 e5                	jne    175b <printf+0x4b>
      if(c == 'd'){
    1776:	83 fa 64             	cmp    $0x64,%edx
    1779:	0f 84 31 01 00 00    	je     18b0 <printf+0x1a0>
        printint(fd, *ap, 10, 1);
        ap++;
      } else if(c == 'x' || c == 'p'){
    177f:	25 f7 00 00 00       	and    $0xf7,%eax
    1784:	83 f8 70             	cmp    $0x70,%eax
    1787:	0f 84 83 00 00 00    	je     1810 <printf+0x100>
        printint(fd, *ap, 16, 0);
        ap++;
      } else if(c == 's'){
    178d:	83 fa 73             	cmp    $0x73,%edx
    1790:	0f 84 a2 00 00 00    	je     1838 <printf+0x128>
          s = "(null)";
        while(*s != 0){
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
    1796:	83 fa 63             	cmp    $0x63,%edx
    1799:	0f 84 35 01 00 00    	je     18d4 <printf+0x1c4>
        putc(fd, *ap);
        ap++;
      } else if(c == '%'){
    179f:	83 fa 25             	cmp    $0x25,%edx
    17a2:	0f 84 e0 00 00 00    	je     1888 <printf+0x178>
#include "user.h"

static void
putc(int fd, char c)
{
  write(fd, &c, 1);
    17a8:	8d 45 e6             	lea    -0x1a(%ebp),%eax
    17ab:	83 c3 01             	add    $0x1,%ebx
    17ae:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
    17b5:	00 
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
        putc(fd, c);
      }
      state = 0;
    17b6:	31 ff                	xor    %edi,%edi
#include "user.h"

static void
putc(int fd, char c)
{
  write(fd, &c, 1);
    17b8:	89 44 24 04          	mov    %eax,0x4(%esp)
    17bc:	89 34 24             	mov    %esi,(%esp)
    17bf:	89 55 d0             	mov    %edx,-0x30(%ebp)
    17c2:	c6 45 e6 25          	movb   $0x25,-0x1a(%ebp)
    17c6:	e8 f7 fd ff ff       	call   15c2 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
        putc(fd, c);
    17cb:	8b 55 d0             	mov    -0x30(%ebp),%edx
#include "user.h"

static void
putc(int fd, char c)
{
  write(fd, &c, 1);
    17ce:	8d 45 e7             	lea    -0x19(%ebp),%eax
    17d1:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
    17d8:	00 
    17d9:	89 44 24 04          	mov    %eax,0x4(%esp)
    17dd:	89 34 24             	mov    %esi,(%esp)
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
        putc(fd, c);
    17e0:	88 55 e7             	mov    %dl,-0x19(%ebp)
#include "user.h"

static void
putc(int fd, char c)
{
  write(fd, &c, 1);
    17e3:	e8 da fd ff ff       	call   15c2 <write>
  int c, i, state;
  uint *ap;

  state = 0;
  ap = (uint*)(void*)&fmt + 1;
  for(i = 0; fmt[i]; i++){
    17e8:	0f b6 53 ff          	movzbl -0x1(%ebx),%edx
    17ec:	84 d2                	test   %dl,%dl
    17ee:	0f 85 76 ff ff ff    	jne    176a <printf+0x5a>
    17f4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
        putc(fd, c);
      }
      state = 0;
    }
  }
}
    17f8:	83 c4 3c             	add    $0x3c,%esp
    17fb:	5b                   	pop    %ebx
    17fc:	5e                   	pop    %esi
    17fd:	5f                   	pop    %edi
    17fe:	5d                   	pop    %ebp
    17ff:	c3                   	ret    
  ap = (uint*)(void*)&fmt + 1;
  for(i = 0; fmt[i]; i++){
    c = fmt[i] & 0xff;
    if(state == 0){
      if(c == '%'){
        state = '%';
    1800:	bf 25 00 00 00       	mov    $0x25,%edi
    1805:	e9 51 ff ff ff       	jmp    175b <printf+0x4b>
    180a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    } else if(state == '%'){
      if(c == 'd'){
        printint(fd, *ap, 10, 1);
        ap++;
      } else if(c == 'x' || c == 'p'){
        printint(fd, *ap, 16, 0);
    1810:	8b 45 d4             	mov    -0x2c(%ebp),%eax
    1813:	b9 10 00 00 00       	mov    $0x10,%ecx
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
        putc(fd, c);
      }
      state = 0;
    1818:	31 ff                	xor    %edi,%edi
    } else if(state == '%'){
      if(c == 'd'){
        printint(fd, *ap, 10, 1);
        ap++;
      } else if(c == 'x' || c == 'p'){
        printint(fd, *ap, 16, 0);
    181a:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
    1821:	8b 10                	mov    (%eax),%edx
    1823:	89 f0                	mov    %esi,%eax
    1825:	e8 46 fe ff ff       	call   1670 <printint>
        ap++;
    182a:	83 45 d4 04          	addl   $0x4,-0x2c(%ebp)
    182e:	e9 28 ff ff ff       	jmp    175b <printf+0x4b>
    1833:	90                   	nop
    1834:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      } else if(c == 's'){
        s = (char*)*ap;
    1838:	8b 45 d4             	mov    -0x2c(%ebp),%eax
        ap++;
    183b:	83 45 d4 04          	addl   $0x4,-0x2c(%ebp)
        ap++;
      } else if(c == 'x' || c == 'p'){
        printint(fd, *ap, 16, 0);
        ap++;
      } else if(c == 's'){
        s = (char*)*ap;
    183f:	8b 38                	mov    (%eax),%edi
        ap++;
        if(s == 0)
          s = "(null)";
    1841:	b8 c0 1a 00 00       	mov    $0x1ac0,%eax
    1846:	85 ff                	test   %edi,%edi
    1848:	0f 44 f8             	cmove  %eax,%edi
        while(*s != 0){
    184b:	0f b6 07             	movzbl (%edi),%eax
    184e:	84 c0                	test   %al,%al
    1850:	74 2a                	je     187c <printf+0x16c>
    1852:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    1858:	88 45 e3             	mov    %al,-0x1d(%ebp)
#include "user.h"

static void
putc(int fd, char c)
{
  write(fd, &c, 1);
    185b:	8d 45 e3             	lea    -0x1d(%ebp),%eax
        ap++;
        if(s == 0)
          s = "(null)";
        while(*s != 0){
          putc(fd, *s);
          s++;
    185e:	83 c7 01             	add    $0x1,%edi
#include "user.h"

static void
putc(int fd, char c)
{
  write(fd, &c, 1);
    1861:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
    1868:	00 
    1869:	89 44 24 04          	mov    %eax,0x4(%esp)
    186d:	89 34 24             	mov    %esi,(%esp)
    1870:	e8 4d fd ff ff       	call   15c2 <write>
      } else if(c == 's'){
        s = (char*)*ap;
        ap++;
        if(s == 0)
          s = "(null)";
        while(*s != 0){
    1875:	0f b6 07             	movzbl (%edi),%eax
    1878:	84 c0                	test   %al,%al
    187a:	75 dc                	jne    1858 <printf+0x148>
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
        putc(fd, c);
      }
      state = 0;
    187c:	31 ff                	xor    %edi,%edi
    187e:	e9 d8 fe ff ff       	jmp    175b <printf+0x4b>
    1883:	90                   	nop
    1884:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
#include "user.h"

static void
putc(int fd, char c)
{
  write(fd, &c, 1);
    1888:	8d 45 e5             	lea    -0x1b(%ebp),%eax
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
        putc(fd, c);
      }
      state = 0;
    188b:	31 ff                	xor    %edi,%edi
#include "user.h"

static void
putc(int fd, char c)
{
  write(fd, &c, 1);
    188d:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
    1894:	00 
    1895:	89 44 24 04          	mov    %eax,0x4(%esp)
    1899:	89 34 24             	mov    %esi,(%esp)
    189c:	c6 45 e5 25          	movb   $0x25,-0x1b(%ebp)
    18a0:	e8 1d fd ff ff       	call   15c2 <write>
    18a5:	e9 b1 fe ff ff       	jmp    175b <printf+0x4b>
    18aa:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
      } else {
        putc(fd, c);
      }
    } else if(state == '%'){
      if(c == 'd'){
        printint(fd, *ap, 10, 1);
    18b0:	8b 45 d4             	mov    -0x2c(%ebp),%eax
    18b3:	b9 0a 00 00 00       	mov    $0xa,%ecx
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
        putc(fd, c);
      }
      state = 0;
    18b8:	66 31 ff             	xor    %di,%di
      } else {
        putc(fd, c);
      }
    } else if(state == '%'){
      if(c == 'd'){
        printint(fd, *ap, 10, 1);
    18bb:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
    18c2:	8b 10                	mov    (%eax),%edx
    18c4:	89 f0                	mov    %esi,%eax
    18c6:	e8 a5 fd ff ff       	call   1670 <printint>
        ap++;
    18cb:	83 45 d4 04          	addl   $0x4,-0x2c(%ebp)
    18cf:	e9 87 fe ff ff       	jmp    175b <printf+0x4b>
        while(*s != 0){
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
        putc(fd, *ap);
    18d4:	8b 45 d4             	mov    -0x2c(%ebp),%eax
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
        putc(fd, c);
      }
      state = 0;
    18d7:	31 ff                	xor    %edi,%edi
        while(*s != 0){
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
        putc(fd, *ap);
    18d9:	8b 00                	mov    (%eax),%eax
#include "user.h"

static void
putc(int fd, char c)
{
  write(fd, &c, 1);
    18db:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
    18e2:	00 
    18e3:	89 34 24             	mov    %esi,(%esp)
        while(*s != 0){
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
        putc(fd, *ap);
    18e6:	88 45 e4             	mov    %al,-0x1c(%ebp)
#include "user.h"

static void
putc(int fd, char c)
{
  write(fd, &c, 1);
    18e9:	8d 45 e4             	lea    -0x1c(%ebp),%eax
    18ec:	89 44 24 04          	mov    %eax,0x4(%esp)
    18f0:	e8 cd fc ff ff       	call   15c2 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
        putc(fd, *ap);
        ap++;
    18f5:	83 45 d4 04          	addl   $0x4,-0x2c(%ebp)
    18f9:	e9 5d fe ff ff       	jmp    175b <printf+0x4b>
    18fe:	66 90                	xchg   %ax,%ax

00001900 <free>:
static Header base;
static Header *freep;

void
free(void *ap)
{
    1900:	55                   	push   %ebp
  Header *bp, *p;

  bp = (Header*)ap - 1;
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1901:	a1 b8 1d 00 00       	mov    0x1db8,%eax
static Header base;
static Header *freep;

void
free(void *ap)
{
    1906:	89 e5                	mov    %esp,%ebp
    1908:	57                   	push   %edi
    1909:	56                   	push   %esi
    190a:	53                   	push   %ebx
    190b:	8b 5d 08             	mov    0x8(%ebp),%ebx
  Header *bp, *p;

  bp = (Header*)ap - 1;
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    if(p >= p->s.ptr && (bp > p || bp < p->s.ptr))
    190e:	8b 08                	mov    (%eax),%ecx
void
free(void *ap)
{
  Header *bp, *p;

  bp = (Header*)ap - 1;
    1910:	8d 53 f8             	lea    -0x8(%ebx),%edx
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1913:	39 d0                	cmp    %edx,%eax
    1915:	72 11                	jb     1928 <free+0x28>
    1917:	90                   	nop
    if(p >= p->s.ptr && (bp > p || bp < p->s.ptr))
    1918:	39 c8                	cmp    %ecx,%eax
    191a:	72 04                	jb     1920 <free+0x20>
    191c:	39 ca                	cmp    %ecx,%edx
    191e:	72 10                	jb     1930 <free+0x30>
    1920:	89 c8                	mov    %ecx,%eax
free(void *ap)
{
  Header *bp, *p;

  bp = (Header*)ap - 1;
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1922:	39 d0                	cmp    %edx,%eax
    if(p >= p->s.ptr && (bp > p || bp < p->s.ptr))
    1924:	8b 08                	mov    (%eax),%ecx
free(void *ap)
{
  Header *bp, *p;

  bp = (Header*)ap - 1;
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1926:	73 f0                	jae    1918 <free+0x18>
    1928:	39 ca                	cmp    %ecx,%edx
    192a:	72 04                	jb     1930 <free+0x30>
    if(p >= p->s.ptr && (bp > p || bp < p->s.ptr))
    192c:	39 c8                	cmp    %ecx,%eax
    192e:	72 f0                	jb     1920 <free+0x20>
      break;
  if(bp + bp->s.size == p->s.ptr){
    1930:	8b 73 fc             	mov    -0x4(%ebx),%esi
    1933:	8d 3c f2             	lea    (%edx,%esi,8),%edi
    1936:	39 cf                	cmp    %ecx,%edi
    1938:	74 1e                	je     1958 <free+0x58>
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
    193a:	89 4b f8             	mov    %ecx,-0x8(%ebx)
  if(p + p->s.size == bp){
    193d:	8b 48 04             	mov    0x4(%eax),%ecx
    1940:	8d 34 c8             	lea    (%eax,%ecx,8),%esi
    1943:	39 f2                	cmp    %esi,%edx
    1945:	74 28                	je     196f <free+0x6f>
    p->s.size += bp->s.size;
    p->s.ptr = bp->s.ptr;
  } else
    p->s.ptr = bp;
    1947:	89 10                	mov    %edx,(%eax)
  freep = p;
    1949:	a3 b8 1d 00 00       	mov    %eax,0x1db8
}
    194e:	5b                   	pop    %ebx
    194f:	5e                   	pop    %esi
    1950:	5f                   	pop    %edi
    1951:	5d                   	pop    %ebp
    1952:	c3                   	ret    
    1953:	90                   	nop
    1954:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  bp = (Header*)ap - 1;
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    if(p >= p->s.ptr && (bp > p || bp < p->s.ptr))
      break;
  if(bp + bp->s.size == p->s.ptr){
    bp->s.size += p->s.ptr->s.size;
    1958:	03 71 04             	add    0x4(%ecx),%esi
    195b:	89 73 fc             	mov    %esi,-0x4(%ebx)
    bp->s.ptr = p->s.ptr->s.ptr;
    195e:	8b 08                	mov    (%eax),%ecx
    1960:	8b 09                	mov    (%ecx),%ecx
    1962:	89 4b f8             	mov    %ecx,-0x8(%ebx)
  } else
    bp->s.ptr = p->s.ptr;
  if(p + p->s.size == bp){
    1965:	8b 48 04             	mov    0x4(%eax),%ecx
    1968:	8d 34 c8             	lea    (%eax,%ecx,8),%esi
    196b:	39 f2                	cmp    %esi,%edx
    196d:	75 d8                	jne    1947 <free+0x47>
    p->s.size += bp->s.size;
    196f:	03 4b fc             	add    -0x4(%ebx),%ecx
    p->s.ptr = bp->s.ptr;
  } else
    p->s.ptr = bp;
  freep = p;
    1972:	a3 b8 1d 00 00       	mov    %eax,0x1db8
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
  if(p + p->s.size == bp){
    p->s.size += bp->s.size;
    1977:	89 48 04             	mov    %ecx,0x4(%eax)
    p->s.ptr = bp->s.ptr;
    197a:	8b 53 f8             	mov    -0x8(%ebx),%edx
    197d:	89 10                	mov    %edx,(%eax)
  } else
    p->s.ptr = bp;
  freep = p;
}
    197f:	5b                   	pop    %ebx
    1980:	5e                   	pop    %esi
    1981:	5f                   	pop    %edi
    1982:	5d                   	pop    %ebp
    1983:	c3                   	ret    
    1984:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    198a:	8d bf 00 00 00 00    	lea    0x0(%edi),%edi

00001990 <malloc>:
  return freep;
}

void*
malloc(uint nbytes)
{
    1990:	55                   	push   %ebp
    1991:	89 e5                	mov    %esp,%ebp
    1993:	57                   	push   %edi
    1994:	56                   	push   %esi
    1995:	53                   	push   %ebx
    1996:	83 ec 1c             	sub    $0x1c,%esp
  Header *p, *prevp;
  uint nunits;

  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
    1999:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
    199c:	8b 1d b8 1d 00 00    	mov    0x1db8,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;

  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
    19a2:	8d 48 07             	lea    0x7(%eax),%ecx
    19a5:	c1 e9 03             	shr    $0x3,%ecx
  if((prevp = freep) == 0){
    19a8:	85 db                	test   %ebx,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;

  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
    19aa:	8d 71 01             	lea    0x1(%ecx),%esi
  if((prevp = freep) == 0){
    19ad:	0f 84 9b 00 00 00    	je     1a4e <malloc+0xbe>
    19b3:	8b 13                	mov    (%ebx),%edx
    19b5:	8b 7a 04             	mov    0x4(%edx),%edi
    base.s.ptr = freep = prevp = &base;
    base.s.size = 0;
  }
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    if(p->s.size >= nunits){
    19b8:	39 fe                	cmp    %edi,%esi
    19ba:	76 64                	jbe    1a20 <malloc+0x90>
    19bc:	8d 04 f5 00 00 00 00 	lea    0x0(,%esi,8),%eax
morecore(uint nu)
{
  char *p;
  Header *hp;

  if(nu < 4096)
    19c3:	bb 00 80 00 00       	mov    $0x8000,%ebx
    19c8:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    19cb:	eb 0e                	jmp    19db <malloc+0x4b>
    19cd:	8d 76 00             	lea    0x0(%esi),%esi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    base.s.size = 0;
  }
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    19d0:	8b 02                	mov    (%edx),%eax
    if(p->s.size >= nunits){
    19d2:	8b 78 04             	mov    0x4(%eax),%edi
    19d5:	39 fe                	cmp    %edi,%esi
    19d7:	76 4f                	jbe    1a28 <malloc+0x98>
    19d9:	89 c2                	mov    %eax,%edx
        p->s.size = nunits;
      }
      freep = prevp;
      return (void*)(p + 1);
    }
    if(p == freep)
    19db:	3b 15 b8 1d 00 00    	cmp    0x1db8,%edx
    19e1:	75 ed                	jne    19d0 <malloc+0x40>
morecore(uint nu)
{
  char *p;
  Header *hp;

  if(nu < 4096)
    19e3:	8b 45 e4             	mov    -0x1c(%ebp),%eax
    19e6:	81 fe 00 10 00 00    	cmp    $0x1000,%esi
    19ec:	bf 00 10 00 00       	mov    $0x1000,%edi
    19f1:	0f 43 fe             	cmovae %esi,%edi
    19f4:	0f 42 c3             	cmovb  %ebx,%eax
    nu = 4096;
  p = sbrk(nu * sizeof(Header));
    19f7:	89 04 24             	mov    %eax,(%esp)
    19fa:	e8 4b fc ff ff       	call   164a <sbrk>
  if(p == (char*)-1)
    19ff:	83 f8 ff             	cmp    $0xffffffff,%eax
    1a02:	74 18                	je     1a1c <malloc+0x8c>
    return 0;
  hp = (Header*)p;
  hp->s.size = nu;
    1a04:	89 78 04             	mov    %edi,0x4(%eax)
  free((void*)(hp + 1));
    1a07:	83 c0 08             	add    $0x8,%eax
    1a0a:	89 04 24             	mov    %eax,(%esp)
    1a0d:	e8 ee fe ff ff       	call   1900 <free>
  return freep;
    1a12:	8b 15 b8 1d 00 00    	mov    0x1db8,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
    if(p == freep)
      if((p = morecore(nunits)) == 0)
    1a18:	85 d2                	test   %edx,%edx
    1a1a:	75 b4                	jne    19d0 <malloc+0x40>
        return 0;
    1a1c:	31 c0                	xor    %eax,%eax
    1a1e:	eb 20                	jmp    1a40 <malloc+0xb0>
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    base.s.size = 0;
  }
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    if(p->s.size >= nunits){
    1a20:	89 d0                	mov    %edx,%eax
    1a22:	89 da                	mov    %ebx,%edx
    1a24:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      if(p->s.size == nunits)
    1a28:	39 fe                	cmp    %edi,%esi
    1a2a:	74 1c                	je     1a48 <malloc+0xb8>
        prevp->s.ptr = p->s.ptr;
      else {
        p->s.size -= nunits;
    1a2c:	29 f7                	sub    %esi,%edi
    1a2e:	89 78 04             	mov    %edi,0x4(%eax)
        p += p->s.size;
    1a31:	8d 04 f8             	lea    (%eax,%edi,8),%eax
        p->s.size = nunits;
    1a34:	89 70 04             	mov    %esi,0x4(%eax)
      }
      freep = prevp;
    1a37:	89 15 b8 1d 00 00    	mov    %edx,0x1db8
      return (void*)(p + 1);
    1a3d:	83 c0 08             	add    $0x8,%eax
    }
    if(p == freep)
      if((p = morecore(nunits)) == 0)
        return 0;
  }
}
    1a40:	83 c4 1c             	add    $0x1c,%esp
    1a43:	5b                   	pop    %ebx
    1a44:	5e                   	pop    %esi
    1a45:	5f                   	pop    %edi
    1a46:	5d                   	pop    %ebp
    1a47:	c3                   	ret    
    base.s.size = 0;
  }
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    if(p->s.size >= nunits){
      if(p->s.size == nunits)
        prevp->s.ptr = p->s.ptr;
    1a48:	8b 08                	mov    (%eax),%ecx
    1a4a:	89 0a                	mov    %ecx,(%edx)
    1a4c:	eb e9                	jmp    1a37 <malloc+0xa7>
  Header *p, *prevp;
  uint nunits;

  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    1a4e:	c7 05 b8 1d 00 00 bc 	movl   $0x1dbc,0x1db8
    1a55:	1d 00 00 
    base.s.size = 0;
    1a58:	ba bc 1d 00 00       	mov    $0x1dbc,%edx
  Header *p, *prevp;
  uint nunits;

  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    1a5d:	c7 05 bc 1d 00 00 bc 	movl   $0x1dbc,0x1dbc
    1a64:	1d 00 00 
    base.s.size = 0;
    1a67:	c7 05 c0 1d 00 00 00 	movl   $0x0,0x1dc0
    1a6e:	00 00 00 
    1a71:	e9 46 ff ff ff       	jmp    19bc <malloc+0x2c>
